...
│   ├── __init__.py
│   ├── data_pipeline.py             # Data extraction, cleaning, and merging (Copernicus + OBIS + Meteo)
│   ├── species_map.py               # Main class SpeciesMap for visualization and interactivity
│   ├── utils.py                     # Helper functions (API requests, caching, formatting)
│   └── analysis.py                  # Optional future module for ecological analysis / stats
│
├── roadmap/
│   └── README.qmd                   # Project outline and Gantt chart (this document)
│
├── slide/
...
│
├── .github/workflows/
│   └── ci.yml                       # GitHub Actions for Continuous Integration (runs unit tests)
│
├── .gitignore
├── requirements.txt
